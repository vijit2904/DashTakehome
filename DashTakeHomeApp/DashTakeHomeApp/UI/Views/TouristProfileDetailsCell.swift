//
//  TouristProfileDetailsCell.swift
//  DashTakeHomeApp
//
...
class TouristProfileDetailsCell: UITableViewCell {
    
    lazy var imgView: UIImageView = {
        let imgView = UIImageView()
        imgView.image = UIImage(systemName: "person.fill")
        imgView.preferredSymbolConfiguration = UIImage.SymbolConfiguration(pointSize: 80)
        return imgView
    }()
    ...
    private func setView() {
        ...
        let imgProfileView = embedinHStack([imgView, vStack])
        addConstraint(for: imgProfileView, contentView: contentView)
    }
